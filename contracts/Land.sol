...
contract Land {
    ...
    constructor() public{
        ...
        addLandInspector("Inspector 1", 45, "Manager");
    }
    ...
    function registerSeller(string memory _name, uint _age, string memory _IDNumber, string memory _phoneNumber, string memory _landsOwned, string memory _document) public {
        ...
        sellersCount++;
        SellerMapping[msg.sender] = Seller(msg.sender, _name, _age, _IDNumber,_phoneNumber, _landsOwned, _document);
        sellers.push(msg.sender);
        emit Registration(msg.sender);
    }

    function updateSeller(string memory _name, uint _age, string memory _IDNumber, string memory _phoneNumber, string memory _landsOwned) public {
        ...
        SellerMapping[msg.sender].IDNumber = _IDNumber;
        SellerMapping[msg.sender].phoneNumber = _phoneNumber;
    ...
    function getSellerDetails(address i) public view returns (string memory, uint, string memory, string memory, string memory, string memory) {
        return (SellerMapping[i].name, SellerMapping[i].age, SellerMapping[i].IDNumber, SellerMapping[i].phoneNumber, SellerMapping[i].landsOwned, SellerMapping[i].document);
    }

    function registerBuyer(string memory _name, uint _age, string memory _city, string memory _IDNumber, string memory _phoneNumber, string memory _document, string memory _email) public {
        ...
        buyersCount++;
        BuyerMapping[msg.sender] = Buyer(msg.sender, _name, _age, _city, _IDNumber, _phoneNumber, _document, _email);
        buyers.push(msg.sender);

        emit Registration(msg.sender);
    }

    function updateBuyer(string memory _name,uint _age, string memory _city,string memory _IDNumber, string memory _email, string memory _phoneNumber) public {
        ...
        BuyerMapping[msg.sender].IDNumber = _IDNumber;
        BuyerMapping[msg.sender].email = _email;
        BuyerMapping[msg.sender].phoneNumber = _phoneNumber;
    ...
    function getBuyerDetails(address i) public view returns ( string memory,string memory, string memory, string memory, string memory, uint, string memory) {
        return (BuyerMapping[i].name,BuyerMapping[i].city , BuyerMapping[i].phoneNumber, BuyerMapping[i].document, BuyerMapping[i].email, BuyerMapping[i].age, BuyerMapping[i].IDNumber);
    }
