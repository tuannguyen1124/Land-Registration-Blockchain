...
class AddLand extends Component {
  ...
  addimage = async () => {
    // alert('In add image')
    const ipfsH1 = await ipfs.add(this.state.buffer);
    this.setState({ ipfsHash: ipfsH1.path });
   
  }
  addDoc = async () => {
    // alert('In add image')
    const ipfsH2 = await ipfs.add(this.state.buffer2);
    this.setState({ document: ipfsH2.path });
  }
